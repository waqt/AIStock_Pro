...
# 每个条目描述的是一个投资概念，不是具体字段名
# ═══════════════════════════════════════════════════════════

FINANCIAL_CONCEPTS_CATALOG = """
## 可查询的数据维度（概念分类）

以下数据可通过工具查询，按投资分析概念分类：

### 资本回报效率 (Capital Return Efficiency)
公司每投入 1 元资本能产生多少回报，是否超过融资成本。
用于判断：商业模式是否真正创造价值、护城河是否转化为超额利润。

### 利润质量与盈利结构 (Profit Quality)
毛利率/净利率水平与趋势、费用结构（研发/销售/管理）。
用于判断：利润是否真实、可持续、议价权在提升还是下降。

### 增长与剪刀差 (Growth & Scissor Gap)
营收增长率 vs 利润增长率，两者趋势是否匹配。
用于判断：增长是靠量价齐升还是降价压货驱动，是否存在增收不增利。

### 财务健康 (Financial Health)
经营现金流与净利润的匹配度、库存周转、合同负债趋势。
用于判断：利润是否真金白银、在手订单是否充足、存货是否积压。

### 竞争护城河稳定性 (Moat Stability)
超额利润是否可持续、利润率是否大起大落、有无财务操纵迹象。
...
def build_synthesis_prompt(
    ...
    for inv in plan.get("investigations", []):
        ...
        if ev.get("search"):
            block += f"搜索信息: {_fmt_json(ev['search'][:600])}\n"

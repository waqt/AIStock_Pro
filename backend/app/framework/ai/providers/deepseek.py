...
class DeepSeekProvider(AIProviderProtocol):
    ...
    async def chat(self, prompt: str) -> Optional[str]:
        ...
        if is_anthropic:
            url = f"{base}/messages"
            body = {"model": settings.DEEPSEEK_MODEL, "max_tokens": 2048, "messages": [{"role": "user", "content": prompt}]}
        else:
            url = f"{base}/chat/completions"
            body = {"model": settings.DEEPSEEK_MODEL, "messages": [{"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 2048}
        headers = {"Authorization": f"Bearer {settings.DEEPSEEK_API_KEY}", "Content-Type": "application/json"}
        async with httpx.AsyncClient(proxy=None, timeout=60.0) as client:
            resp = await client.post(url, json=body, headers=headers)
            if resp.status_code != 200:
